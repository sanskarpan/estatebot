...
| **Availability check** | `https://github.com/sanskarpan/estatebot/actions/workflows/availability-ping.yml` (15-minute schedule configured; public probe manually verified) |
...
- The free host can sleep during inactivity, so the first request after an idle period can be slower. A best-effort fifteen-minute health check is configured; manual and later schedule-triggered health probes passed, although GitHub scheduling can be delayed or skip slots. The UI therefore retains a bounded waking/retry path. Neither mechanism is an uptime guarantee, and a full idle-window wake was not observed during the release window.
