...
- The free host can sleep during inactivity, so the first request after an idle period can be slower. A best-effort fifteen-minute health check is configured and its health request is manually verified, but no schedule-triggered run appeared during three observed slots; the UI therefore retains a bounded waking/retry path. Neither mechanism is an uptime guarantee, and a full idle-window wake was not observed during the release window.
