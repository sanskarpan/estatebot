...
DarGlobal's ordinary HTTP path returned an Incapsula shell during this build, while the same public pages loaded in a standard browser session without authentication or challenge solving. The auditable browser capture contains 36 project pages and 15 dated press articles at `data/darglobal_browser_capture.json` and is normalized with:
