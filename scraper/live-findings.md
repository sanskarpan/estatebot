...
| `wasalt.sa` | `robots.txt` was publicly readable and allows `/` for the default user-agent while disallowing `/search` and `/cdn-cgi/rum`. Listing landing/detail HTTP probes returned Cloudflare challenge responses (`403`, `cf-mitigated: challenge`). The normal unauthenticated browser path exposed an 80-result public Projects search and 32 project detail links in its initial rendered result set. | Use the permitted public sitemap as a bounded listing-discovery fallback and never solve the Cloudflare challenge. Capture the 32 visible public project pages through the normal browser path and normalize their project facts with `ingestion.import_wasalt_projects`; Auctions is a separate host/surface and remains out of scope. |
...
Current normalized snapshot: 36 DarGlobal projects, 15 dated DarGlobal press releases, 2 DarGlobal company documents, 180 Wasalt sale/rent listings, 32 Wasalt projects, and 3 Wasalt city-guide documents.
