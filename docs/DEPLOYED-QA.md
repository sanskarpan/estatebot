...
- The fifteen-minute health check is configured and its public health request passed by manual dispatch and later scheduled runs. The free host's complete idle/sleep window was not waited out, so this remains best-effort mitigation rather than an uptime claim.
...
- The first post-registration observation window did not show a run at the expected 21:05, 21:20, or 21:35 UTC slots, including a five-minute allowance after the last slot. GitHub later produced successful scheduled runs `33695404708` (23:30 UTC), `33703338082` (01:21 UTC), and `33722825739` (06:21 UTC), each validating 248 records. This verifies that scheduling eventually fires, but not that every fifteen-minute slot executes or that the service stays continuously warm.
...
- A live browser reproduction found that valid remote images were loading but rendered as short top strips because the media frame had only `min-height`; the image's percentage height had no definite containing block. The card media frame now has explicit desktop/mobile heights and absolutely fills the frame with `object-fit: cover`. Property-card contract coverage was extended, and the full 75-test suite passed locally before release.
