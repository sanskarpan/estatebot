...
> **Post-release addendum (2026-09-02):** The implementation is complete and verified locally and at its public HTTPS endpoint. It includes the runnable system, fixtures, Docker/CI configuration, authenticated free-model inference, and a two-source seed containing 36 DarGlobal projects, 15 DarGlobal press releases, 2 DarGlobal company documents, 180 Wasalt sale/rent listings, 32 Wasalt projects, and 3 Wasalt city guides. The original review below is intentionally retained as the pre-build audit; current evidence is maintained in [`IMPLEMENTATION-STATUS.md`](IMPLEMENTATION-STATUS.md) and [`DEPLOYED-QA.md`](DEPLOYED-QA.md).

## Post-build resolution summary

The original critical findings are resolved as follows: all referenced runtime files now exist (C1); unknown/inactive records use `404`/`410` (C2); listing lookup and search are implemented (C3); persistence, lookup, deactivation, retrieval, and citations use composite source identity (C4); incomplete/blocked runs cannot mass-deactivate a source (C5); deterministic structured, lexical, named-entity, cross-source, no-match, and press-routing paths are implemented and tested (C6); model fallback timeouts, bounded context assembly, and live authenticated inference are verified (C7); SSE buffers and verifies before final delivery (C8); and clean Docker startup bootstraps SQLite/FTS5 from the checked-in seed (C9).
...
The external release proof is now complete: the public repository, HTTPS service, authenticated model chain, and deployed QA are recorded in [`../SUBMISSION.md`](../SUBMISSION.md). Only reviewer submission itself remains an external human action.
