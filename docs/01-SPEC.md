...
| 4 | Containerise the application using Docker | A. A `Dockerfile` (or multi-stage `Dockerfile`) builds the backend+frontend (or each service) into a runnable image. B. A `docker-compose.yml` runs the full stack (API, static frontend or SSR, vector store if separate, scraper as a one-off job) with a single `docker compose up`. C. Image builds reproducibly from a clean checkout with no manual steps. |
...
- FR-7: All scraped text is chunked, embedded, and stored in a vector index (or, if resource-constrained, an equivalent structured+keyword hybrid search — see `docs/05-CHATBOT-RAG-SPEC.md` §Retrieval strategy options and the fallback path for zero-budget embeddings).
...
- FR-11: Every generated answer that references specific properties includes machine-checkable citations (listing IDs / URLs) that the backend can verify actually exist in the corpus before sending the response (reject/regenerate if the model cites something not retrieved — see `docs/05-CHATBOT-RAG-SPEC.md` §Citation verification).
...
|---|---|
| Performance | P50 chat response < 6s, P95 < 15s, measured end-to-end from `POST /api/chat` request to full non-streamed response (streaming: time-to-first-token < 3s P50). Documented free-model latency variance is acceptable but must degrade gracefully, not hang indefinitely (hard timeout, see `docs/05-CHATBOT-RAG-SPEC.md` §Timeouts). |
| Availability | Deployed URL responds during the review window. `/api/health` used for basic uptime self-check. Free hosting tiers that "sleep" after inactivity are acceptable **only if** cold-start is < 30s and the UI shows a "waking up" state — otherwise pick a host that doesn't sleep (see `docs/08-DEPLOYMENT.md` §Hosting comparison). |
...
| Security | No secrets committed to the repo. API keys only via environment variables / host secret manager. Basic input sanitization against prompt injection and XSS in rendered chat content (escape/sanitize markdown-to-HTML). Rate limiting on `/api/chat` to control cost/abuse (see `docs/06-API-SPEC.md` §Rate limiting). |
| Cost | Entire stack must run within free tiers: OpenRouter free model, free/low-cost hosting, free or self-hosted embeddings, free or self-hosted vector store. Documented explicitly in `docs/08-DEPLOYMENT.md` §Cost ledger. |
...
- Assume no paid API budget: OpenRouter free model, free/local embeddings (e.g. `sentence-transformers` running in the container, or an OpenRouter/other provider's free embedding endpoint if one is available — verify at build time, do not assume), free vector store (e.g. embedded Chroma/FAISS/SQLite+vector extension) or a generous free tier (e.g. hosted vector DB free tier) — pick and justify one option in `docs/02-ARCHITECTURE.md`.
