...
| 3.10 | Model context window smaller than the default chunk/history budget (a small free model) | Dynamically reduce top-k and history turns based on the actually-selected model's real `context_length` (§2.3 in `docs/05-CHATBOT-RAG-SPEC.md`), not a hardcoded assumption. |
